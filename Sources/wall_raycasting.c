...
/*   Updated: 2016/09/21 18:56:55 by atoulous         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	perform_dda(t_var *var)
{
	...
	while (HIT == 0)
	{
		...
		if (TAB[MAP_X][MAP_Y] != '0' && TAB[MAP_X][MAP_Y] != 'R'
				...
				&& TAB[MAP_X][MAP_Y] != 'p' && TAB[MAP_X][MAP_Y] != 'u')
			HIT = 1;
	}
}
...
{
	POS == '1' ? COLOR = WALLDATA[px] + WALLDATA[px + 1] * 256 +
		WALLDATA[px + 2] * 65536 : 0;
	POS == '2' ? COLOR = WALLTAGDATA[px] + WALLTAGDATA[px + 1] * 256 +
		WALLTAGDATA[px + 2] * 65536 : 0;
	POS == 'F' ? COLOR = WINWALLDATA[px] + WINWALLDATA[px + 1] * 256 +
		WINWALLDATA[px + 2] * 65536 : 0;
	POS == 'f' ? COLOR = WINTAGDATA[px] + WINTAGDATA[px + 1] * 256 +
		WINTAGDATA[px + 2] * 65536 : 0;
	POS == 'D' || POS == 'd' ? COLOR = DOORWALLDATA[px] + DOORWALLDATA[px + 1] *
		256 + DOORWALLDATA[px + 2] * 65536 : 0;
	POS == 'I' ? COLOR = INDOORDATA[px] + INDOORDATA[px + 1] * 256 +
		INDOORDATA[px + 2] * 65536 : 0;
	POS == 'T' ? COLOR = PORTALDATA[px] + PORTALDATA[px + 1] * 256 +
		PORTALDATA[px + 2] * 65536 : 0;
	POS == 'B' ? COLOR = BOXDATA[px] + BOXDATA[px + 1] * 256 +
		BOXDATA[px + 2] * 65536 : 0;
	POS == 'b' ? COLOR = METALBOXDATA[px] + METALBOXDATA[px + 1] * 256 +
		METALBOXDATA[px + 2] * 65536 : 0;
	POS == 'A' || POS == 'U' ? COLOR = AUTOMATDATA[px] + AUTOMATDATA[px + 1] *
		256 + AUTOMATDATA[px + 2] * 65536 : 0;
	POS == 'M' ? COLOR = OIMDATA[px] + OIMDATA[px + 1] * 256 +
		OIMDATA[px + 2] * 65536 : 0;
}
...
void	draw_wall(t_var *var, int x)
{
	...
	while (Y1 <= DRAWEND)
	{
		...
		if ((SIDE == 0 && RAYDIR_X > 0) || (SIDE == 1 && RAYDIR_Y < 0))
			texx = (TEXTX - texx - 1);
		D & (1 << 0) ?
			(d = Y1 * 256 - HEIGHT_WIN * 128 +
			(LINEHEIGHT * (2 / DUCKEND)) * 128) :
...
void	wall_raycasting(t_var *var, int x)
{
	...
	DRAWSTART = (HEIGHT_WIN / 2) - LINEHEIGHT * (1 / DUCK);
	DRAWSTART < 0 ? DRAWSTART = 0 : 0;
	DRAWEND = (HEIGHT_WIN / 2) + LINEHEIGHT * (2 / DUCKEND);
